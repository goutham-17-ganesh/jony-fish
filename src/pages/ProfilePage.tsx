...
export default function ProfilePage() {
  ...
  const handleComment = async (postId: string) => {
    ...
    try {
      const sampleComments = [
        t('profile.sampleComment1'),
        t('profile.sampleComment2'),
        t('profile.sampleComment3'),
        t('profile.sampleComment4'),
      ];
  ...
  const handleShare = (postId: string) => {
    ...
    if (navigator.share) {
      navigator.share({
        title: t('profile.shareTitle', { species: post.species }),
        text: post.caption,
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto max-w-md p-4 space-y-6">
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <Badge variant="secondary" className="flex items-center gap-1">
                ...
                {t('profile.favorite')}: {currentUser.favoriteSpecies}
              </Badge>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">{t('profile.myPosts')}</CardTitle>
          </CardHeader>
          <CardContent>
            {userPosts.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground" data-testid="text-no-posts">
                ...
                <p className="text-sm">{t('profile.shareFirstCatch')}</p>
              </div>
            ) : (
